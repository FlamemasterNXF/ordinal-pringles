...
let hierarchyData = [
    { text:"Multiplying Incrementy Gain by", effect: ()=> Math.max((Math.log10(data.hierachies.ords[0].ord+1)*hbData[1].effect())**(dupEffect(2)), 1),
        gain: ()=> hierarchyGainBases[0]()*hierarchyGainGlobalMults() },
    { text:"Dividing Charge Requirement by", effect: ()=> Math.max((Math.log10(data.hierachies.ords[1].ord+1)*hbData[4].effect()*alephEffect(5))**(dupEffect(2)), 1),
...
let hupData = [
    ...
    { text:"Total Charge Boosts AutoBuyers", cost: 1e10, effect: ()=> data.hierachies.hasUpgrade[3] ? Math.max(1, data.incrementy.totalCharge/2) : 1 },
...
function buyHBuyable(i){
    ...
    if(data.hierachies.ords[1].ord > cost && i > 2 && i < 5){
