...
function psiHardy(ord, base) {
    ...
    if (!capOrdinalAtBO && D(ord.layer).gte(Decimal.tetrate(PSI_VALUE,3).mul(3).sub(6).add(D(ord.mag).gte(D(BO_VALUE).mag)?0:1))) return "s(3,3{1,,1,,2}2)"; // OFP = I (SAN)
    if (!capOrdinalAtBO && D(ord.layer).gte((PSI_VALUE*3)-6+(D(ord.mag).gte(D(BO_VALUE).mag)?0:1))) return "s(3,3{1,,1``2}2)"; // Ω_Ω₂ (SAN)
    if (!capOrdinalAtBO && D(ord.layer).gte((27*3)-6+(D(ord.mag).gte(D(BO_VALUE).mag)?0:1))) return "s(3,3{1,,1{1`2`}2}2)"; // Ω_{Ω^Ω} (SAN)
    if (!capOrdinalAtBO && D(ord.layer).gte((9*3)-6+(D(ord.mag).gte(D(BO_VALUE).mag)?0:1))) return "s(3,3{1,,1`1`2}2)"; // Ω_{Ω²} (SAN)
    if (!capOrdinalAtBO && D(ord.layer).gte((6*3)-6+(D(ord.mag).gte(D(BO_VALUE).mag)?0:1))) return "s(3,3{1,,1`3}2)"; // Ω_{Ω2} (SAN)
    if (!capOrdinalAtBO && D(ord.layer).gte((4*3)-6+(D(ord.mag).gte(D(BO_VALUE).mag)?0:1))) return "s(3,3{1,,2`2}2)"; // Ω_{Ω+1} (SAN)
    if (!capOrdinalAtBO && D(ord.layer).gte((3*3)-6+(D(ord.mag).gt(D(BO_VALUE).mag)?0:1))) return "s(3,3{1,,1`2}2)"; // Ω_Ω (SAN)
