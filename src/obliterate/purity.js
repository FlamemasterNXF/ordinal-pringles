...
function setupPurityPoint(i, point){
    ...
    point.addEventListener('mouseover', (e) => displayPringleButton(e, null, i))
    point.addEventListener('click', () => assignPringle(i, 1))
}

function updatePurityText(i) {
    let pringle = getPurityPringleData(i)
    const target = !isMobileMode() ? 'pringleButton' : 'mobilePurityInfoText'

    DOM(target).innerHTML = isPurityPointUnlocked(i) ?
