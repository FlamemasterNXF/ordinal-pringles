...
function initAlephs() {
    ...
    for (let i = 0; i < data.collapse.alephs.length; i++) {
        ...
        boostManager.register({
            name: `ℵ${i+1}`,
            target: alephData[i].target,
            sign: 'x',
            color: graphColors.cardinal,
            effect: () => getAlephEffect(i),
            shouldDisplay: () => data.collapse.times > 0 || data.obliterate.times > 0
        })
    }
}
...
function initCUPS() {
    ...
    for (let i = 0; i < data.collapse.hasCUP.length / 4; i++) {
        ...
        for (let j = 0; j < 4; j++) {
            ...
            boostManager.register({
                name: `CUP${id+1}`,
                target: cupData[id].target,
                sign: id === 1 ? '^' : 'x',
                color: graphColors.cardinal,
                effect: () => getCUPEffect(id, false),
                shouldDisplay: () => hasCUP(id)
            })
        }
    }
    ...
    switch (mode) {
        ...
        case 1:
            if (data.boost.times <= sluggishData[i].req && !hasSluggishMilestone(i)) {
                ...
                createNotification(`You have completed a Sluggish Milestone${data.collapse.times > 1 ? `and your completion has been rewarded with ${3 * i} free Cardinals!` : '!'}`)
...
function cardinalGain() {
    ...
    let gain = Decimal.sqrt(data.boost.times - 34)
        ...
        .times(alephTotalEffect()).times(getRUPEffect(5)).times(getAOMEffect(4)).times(getPringleEffect(2))
...
let alephData = [
    {
        text: "multiplying Autoclickers by",
        effect: () => Decimal.sqrt(getAlephAmount(0).plus(1)).times(3).times(purificationEffect(1)),
        unl: () => true,
        target: 'All AutoClickers'
    },
    {
        text: "multiplying Autobuyers by",
        effect: () => Decimal.log10(D(10).plus((getAlephAmount(1).times(90)))).times(purificationEffect(1)),
        unl: () => true,
        target: 'All AutoBuyers'
    },
    {
        text: "multiplying Ordinal Power gain by",
        effect: () => Decimal.log2(getAlephAmount(2).plus(2)).times(3),
        unl: () => true,
        target: 'OP'
    },
    {
        text: "multiplying Incrementy gain by",
        effect: () => Decimal.pow(getAlephAmount(3).plus(1), 1 / 4),
        unl: () => true,
        target: 'Incrementy Gain'
    },
    {
        text: "multiplying Dynamic Cap by",
        effect: () => ((Decimal.sqrt(getAlephAmount(4).plus(1)).times(2)).plus(hupData[9].effect())).times(getAOMEffect(1)),
        unl: () => true,
        target: 'Dynamic Cap'
    },
    {
        text: "multiplying the SGH effect by",
        effect: () => Decimal.pow(getAlephAmount(5).plus(1), 1 / 4),
        unl: () => true,
        target: 'SGH Effect'
    },
    {
        text: "multiplying Booster Power gain by",
        effect: () => Decimal.sqrt(getAlephAmount(6).plus(4)).div(2),
        unl: () => true,
        target: 'Booster Power'
    },
    {
        text: "multiplying the IUP3 effect by",
        effect: () => (Decimal.sqrt(getAlephAmount(7).plus(4)).times(2).times(purificationEffect(1))).plus(hupData[9].effect()),
        unl: () => true,
        target: 'IUP3'
    },
    {
        text: "multiplying Dynamic Gain by",
        effect: () => Decimal.log2(getAlephAmount(8)),
        unl: () => hasAOMilestone(1),
        target: 'Dynamic Gain'
    },
]
let cupData = [
    {
        ...
        cost: 9,
        effect: () => Math.max((data.incrementy.totalCharge / 2) * purificationEffect(3) * getPringleEffect(0, true), 1),
        target: 'All AutoBuyers'
    },
    {
        text: "Square AutoClicker speeds",
        cost: 27,
        effect: () => 2 + getPringleEffect(1, true),
        target: 'All AutoClickers'
    },
    {
        text: "Challenge Completions multiply AutoBuyer speed",
        cost: 81,
        effect: () => 1 + getTotalChallengeCompletions(),
        target: 'All AutoBuyers'
    },
    {
        text: "Ordinal Powers boost AutoBuyers and AutoClickers",
        cost: 243,
        effect: () => Math.max(1, Decimal.log10(data.markup.powers.plus(1)).div(10).toNumber()),
        target: 'All Automation'
    },
    {
        text: "Incrementy boosts its own gain",
        cost: 2187,
        effect: () => Math.min(Decimal.max(1, Decimal.log10(data.incrementy.amt.plus(1))).mul(purificationEffect(3)).mul(getPringleEffect(0)).toNumber(), Number.MAX_VALUE),
        target: 'Incrementy Gain'
    },
    {
        text: "Unlock a 3rd Overcharge Effect and boost Overcharge's 1st Effect",
        cost: 196608,
        effect: () => 3 * getPringleEffect(0, true),
        target: 'Overcharge Effect 1'
    },
    {
        text: "Unspent Cardinals boost Alephs",
        cost: 3e9,
        effect: () => Decimal.max(1, Decimal.log2(data.collapse.cardinals).times(getAOMEffect(3)).times(getPringleEffect(2))),
        target: 'All Alephs'
    },
    {
        text: "Gain a percent of best Cardinals gained on Collapse every second",
        cost: 1e13,
        effect: () => getAOREffect(7),
        target: 'Cardinals'
    },
]
let sluggishData = [
    {
        text: "Uncap the Ordinal, you can pass Graham's Number without Boosting, gain 1% of Ordinal Powers gained on Markup every second, and you always have one free Maximize and Successor AutoClicker",
        req: 34
    },
    {
        text: "Keep Challenges and Incrementy unlocked through Collapse",
        req: 29
    },
    {
        text: "Unlock an AutoBuyer for Charge, an AutoBuyer for RUP1-3, and Unlock Darkness",
        req: 24
    },
    {
        text: "Unlock an AutoBuyer for Repeatable Hierarchy Upgrades, AutoPrestigers for Factor Shift and Factor Boost, keep UP1-6 on Collapse, and unlock a new row of Booster Upgrades",
        req: 12
    },
    {
        text: "Unlock 4 new Hierarchy Upgrades, keep Hierarchies unlocked through Collapse, and keep Challenge completions on Collapse",
        req: 2
    },
]
...
let collapseConfirm = (auto = false) =>
    getSimpleSetting('collapseConfirmation')
        ? createConfirmation('Are you certain?', `Collapsing will reset everything prior!`, 'No Way!', 'Go Ahead!', collapse)
        : collapse(false, auto)
...
function collapse(first = false, auto = false) {
    ...
    if (!auto) createNotification("Insufficent Ordinal")
}
...
function collapseReset() {
    ...
    data.incrementy.hasIUP = Array(6).fill(hasSluggishMilestone(3))
...
function collapseCardinals() {
    if (data.collapse.cardinals.eq(0)) return createNotification("No Cardinals to Collapse")
...
boostManager.register({
    name: 'All Alephs',
    target: ['ℵ1', 'ℵ2', 'ℵ3', 'ℵ4', 'ℵ5', 'ℵ6', 'ℵ7', 'ℵ8', 'ℵ9'],
    color: graphColors.cardinal,
    shouldDisplay: () => data.collapse.times > 0 || data.obliterate.times > 0
})
