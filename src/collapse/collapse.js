function updateCollapseHeaderHTML(){
    if(getSubtab('collapse') === 'cUpgrades' && hasSluggishMilestone(2)){
        ...
        DOM('cardinalsText').innerHTML = `<span style="color: goldenrod">You have ${formatWhole(data.incrementy.charge)} Charge</span>${data.obliterate.times > 0 ? ` <span style="color: #b87dd9">and ${data.stability.energy[0]} Stable Energy</span>` : ''}<br><span style="font-size: 0.9rem; color: #8a8a8a">You have ${getStableHypercharges()} Stable Hypercharges, <span style="color: #20da45">raising Incrementy gain to the ${format(getStableHyperchargeEffect())}</span></span>`
    }
