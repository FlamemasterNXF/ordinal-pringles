...
function updateMarkupHTML(){
    ...
    DOM("factorBoostButton").innerHTML = `Perform ${getBulkBoostAmt() < 2 ? "a Factor Boost" : getBulkBoostAmt()+" Factor Boosts"} [+${inOC(0) ? getBulkBoostAmt() : boostersAtGivenFB(data.boost.times+getBulkBoostAmt())-boostersAtGivenFB(data.boost.times)}] (B)<br>Requires ${displayPsiOrd(boostReq(), 3)}`
